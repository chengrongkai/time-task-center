...
import com.crk.timetaskcenter.common.exception.code.BaseResponseCode;
import com.crk.timetaskcenter.common.job.ScheduleJob;
...
public class TimeTaskController {
    ...
    public DataResult add(@RequestBody TimeTaskEntity sysJob) {
        ...
        timeTaskService.saveJob(sysJob);
    ...
    public DataResult update(@RequestBody TimeTaskEntity sysJob) {
        if (isValidExpression(sysJob.getCronExpression())) {
            return DataResult.fail("cron表达式有误");
        }
        DataResult dataResult = ScheduleJob.judgeBean(sysJob.getBeanName());
        ...
        timeTaskService.updateJobById(sysJob);
        return DataResult.success();
    }
    ...
    public DataResult findListByPage(@RequestBody TimeTaskEntity sysJob) {
        Page page = new Page(sysJob.getPage(), sysJob.getLimit());
        LambdaQueryWrapper<TimeTaskEntity> queryWrapper = Wrappers.lambdaQuery();
        //查询条件示例
        if (!StringUtils.isEmpty(sysJob.getBeanName())) {
            queryWrapper.like(TimeTaskEntity::getBeanName, sysJob.getBeanName());
        }
        IPage<TimeTaskEntity> iPage = timeTaskService.page(page, queryWrapper);
